...
public class ScaffoldBasedMudBlock extends Block {
	 private static final VoxelShape NORMAL_OUTLINE_SHAPE;
	   ...
	   public ScaffoldBasedMudBlock(AbstractBlock.Settings settings) {
	      super(settings);
	      this.setDefaultState((BlockState)((BlockState)((BlockState)((BlockState)this.stateManager.getDefaultState()).with(DISTANCE, 7))).with(BOTTOM, false));
	   }
	   ...
	   public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
	      ...
	      if ((Integer)blockState.get(DISTANCE) == 7) {
	         if ((Integer)state.get(DISTANCE) == 7) {
	            world.spawnEntity(new FallingBlockEntity(world, (double)pos.getX() + 0.5D, (double)pos.getY(), (double)pos.getZ() + 0.5D, (BlockState)blockState));
	         } else {
